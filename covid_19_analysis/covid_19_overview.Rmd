...
country_per_captia_avg.coronavirus <- 
  merge(country_per_captia_avg.coronavirus, pop, by.x = "country", by.y = "name") %>% 
  select(
    country,
    avg,
    pop_2020 = "2020"
...
head(country_per_captia_avg.coronavirus)
```

Some countries seem to be missing from the newly created dataset.
The problem lies with the fact that `pop` and `coronavirus` use different names for the same country.